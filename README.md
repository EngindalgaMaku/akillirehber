...
AkıllıRehber'in başarımı, Bloom Taksonomisi'ne göre sınıflandırılmış 100 adet Türkçe soru-cevap çiftinden oluşan özgün bir veri seti ile değerlendirilmiştir. Değerlendirme üç ana deney grubundan oluşmaktadır:

### Deney 1: RAGAS Değerlendirmesi

RAG pipeline'ının uçtan uca performansını ölçer:
...
| Context Precision | Getirilen bağlamın hassasiyeti |
| Context Recall | Getirilen bağlamın kapsamı |
| Answer Correctness | Yanıtın doğruluğu |

### Deney 2: ROUGE ve BERTScore

Üretilen yanıtların referans yanıtlarla metin benzerliğini ölçer:
- ROUGE-1, ROUGE-2, ROUGE-L (n-gram tabanlı benzerlik)
- BERTScore Precision, Recall, F1 (anlamsal benzerlik)

### Deney 3: RAG vs Direct LLM Karşılaştırması

Aynı sorular üzerinde RAG tabanlı yanıtlar ile yalın LLM yanıtlarını karşılaştırır. Bu deney, RAG mimarisinin ders materyallerine dayalı yanıt üretimindeki katkısını ortaya koymaktadır.

### Deney 4: PII Filtreleme Performansı

KVKK uyumlu iki katmanlı kişisel bilgi filtreleme sisteminin precision, recall ve F1 skorlarını ölçer:
- Katman 1 (Regex): TC kimlik, telefon, e-posta, IBAN, kredi kartı gibi yapısal kalıplar
- Katman 2 (Few-Shot Embedding): Şifre, adres gibi belirsiz durumlar için k-NN sınıflandırma

### Deneyleri Tekrarlama

Deney betikleri [`experiments/`](./experiments/) dizininde yer almaktadır:
...
Detaylı bilgi için: [`experiments/README.md`](./experiments/README.md)

### Değerlendirme Veri Seti

Bloom Taksonomisi'ne göre sınıflandırılmış 100 adet Türkçe soru-cevap çifti:
https://github.com/EngindalgaMaku/Bilisim-Teknolojileri-Bloom-Dataset
...
Sorularınız için issue açabilir veya [2430131010@ogr.mehmetakif.edu.tr](mailto:2430131010@ogr.mehmetakif.edu.tr) adresinden iletişime geçebilirsiniz.
