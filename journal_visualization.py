...
def load_config_data():
    ...
    try:
        ...
        print("Warning: Config 2 CSV file not found. Using placeholder values.")
    ...
    try:
        ...
        print("Warning: Config 3 CSV file not found. Using placeholder values.")
        # Placeholder values if CSV not available
        data3_baseline = {
...
def plot_1_grouped_bar_comparison(df_plot):
    """Figure 1: Grouped bar chart comparing all metrics across configurations."""
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('Performance Comparison: Baseline vs Reranker Across Configurations', 
...
def plot_2_improvement_heatmap(all_data):
    ...
    improvements = []
    for config_key, config_name in [(k, v) for k, v in 
                                    [('config1', 'Config 1 (Jina)'), 
                                     ('config2', 'Config 2 (OpenAI)'),
                                     ('config3', 'Config 3 (Cohere)')]]:
...
def plot_3_radar_chart(all_data):
    """Figure 3: Radar chart comparing all metrics for each configuration."""
    metrics = ['ROUGE-1', 'ROUGE-2', 'ROUGE-L', 'BERTScore']
        ...
        if baseline and rerank:
            ...
            for angle, bl_val, rr_val in zip(angles[:-1], baseline_values[:-1], rerank_values[:-1]):
                ax.text(angle, bl_val + 0.02, f'{bl_val:.3f}', 
                       ha='center', va='center', fontsize=7, color=COLORS['baseline'])
                ax.text(angle, rr_val - 0.02, f'{rr_val:.3f}', 
                       ha='center', va='center', fontsize=7, color=COLORS['rerank'])
...
def plot_5_comprehensive_summary(df_plot, all_data):
    ...
    bars = ax2.barh(range(len(best_config_sorted)), best_config_sorted.values, 
                   color=colors)
    ...
    for metric in ['ROUGE-1', 'ROUGE-2', 'ROUGE-L', 'BERTScore']:
        baseline_avg = df_plot[(df_plot['Metric'] == metric) & 
                              (df_plot['Type'] == 'Baseline')]['Score'].mean()
        rerank_avg = df_plot[(df_plot['Metric'] == metric) & 
                            (df_plot['Type'] == 'Rerank')]['Score'].mean()
    ...
    bars = ax3.bar(metric_improvements.keys(), metric_improvements.values(), 
    ...
    for i, label in enumerate(config_labels):
        ax4.annotate(label.split('\n')[0], (latency_data[i], performance_data[i]),
    ...
    avg_latency_red = np.mean([((rerank_latencies[i] - baseline_latencies[i]) / 
...
def generate_summary_report(all_data, df_plot):
    ...
    for config_key, config_name in [(k, v) for k, v in 
        ...
        if baseline and rerank:
            ...
            report.append(f"  Improvement: {calculate_improvement(baseline['aggregate/avg_rougel'], rerank['aggregate/avg_rougel']):.2f}%")
            ...
            report.append(f"  Improvement: {calculate_improvement(baseline['aggregate/avg_bertscore_f1'], rerank['aggregate/avg_bertscore_f1']):.2f}%")
    ...
    for config_key, config_name in [(k, v) for k, v in 
        ...
        if baseline and rerank:
            latency_change = calculate_improvement(baseline['latency_ms'], 
                                                   rerank['latency_ms'])
            report.append(f"\n{config_name}:")
