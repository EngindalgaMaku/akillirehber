...
def load_config_data():
    ...
    try:
        ...
        if 'Baseline' in summary2.index:
            data2_baseline = {
                'aggregate/avg_rouge1': summary2.loc['Baseline', 'aggregate/avg_rouge1'],
                'aggregate/avg_rouge2': summary2.loc['Baseline', 'aggregate/avg_rouge2'],
                'aggregate/avg_rougel': summary2.loc['Baseline', 'aggregate/avg_rougel'],
                'aggregate/avg_bertscore_f1': summary2.loc['Baseline', 'aggregate/avg_bertscore_f1'],
                'latency_ms': summary2.loc['Baseline', 'latency_ms']
            }
        if 'Rerank' in summary2.index:
            data2_rerank = {
                'aggregate/avg_rouge1': summary2.loc['Rerank', 'aggregate/avg_rouge1'],
                'aggregate/avg_rouge2': summary2.loc['Rerank', 'aggregate/avg_rouge2'],
                'aggregate/avg_rougel': summary2.loc['Rerank', 'aggregate/avg_rougel'],
                'aggregate/avg_bertscore_f1': summary2.loc['Rerank', 'aggregate/avg_bertscore_f1'],
    ...
    try:
        ...
        if 'Baseline' in summary3.index:
            data3_baseline = {
                'aggregate/avg_rouge1': summary3.loc['Baseline', 'aggregate/avg_rouge1'],
                'aggregate/avg_rouge2': summary3.loc['Baseline', 'aggregate/avg_rouge2'],
                'aggregate/avg_rougel': summary3.loc['Baseline', 'aggregate/avg_rougel'],
                'aggregate/avg_bertscore_f1': summary3.loc['Baseline', 'aggregate/avg_bertscore_f1'],
                'latency_ms': summary3.loc['Baseline', 'latency_ms']
            }
        if 'Rerank' in summary3.index:
            data3_rerank = {
                'aggregate/avg_rouge1': summary3.loc['Rerank', 'aggregate/avg_rouge1'],
                'aggregate/avg_rouge2': summary3.loc['Rerank', 'aggregate/avg_rouge2'],
                'aggregate/avg_rougel': summary3.loc['Rerank', 'aggregate/avg_rougel'],
                'aggregate/avg_bertscore_f1': summary3.loc['Rerank', 'aggregate/avg_bertscore_f1'],
...
def plot_grouped_bar_comparison(df_plot):
    """Figure 1: Grouped bar chart comparing all metrics across configs."""
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('Performance Comparison: Baseline vs Reranker',
    ...
    for idx, metric in enumerate(metrics):
        ...
        baseline_scores = metric_data[metric_data['Type'] == 'Baseline']['Score'].values
        rerank_scores = metric_data[metric_data['Type'] == 'Rerank']['Score'].values
        ...
        bars2 = ax.bar(x + width/2, rerank_scores, width,
                       label='Rerank', color=COLORS['rerank'], alpha=0.8)
    ...
    plt.tight_layout()
    plt.savefig('figure1_grouped_comparison.png', bbox_inches='tight', dpi=300)
    plt.close()
    print("✓ Figure 1: Grouped bar comparison saved")


def plot_improvement_heatmap(all_data):
    ...
    for config_key, config_name in config_names:
        ...
        if baseline and rerank:
            ...
            for metric_key, _ in metric_keys:
                improvement = calculate_improvement(baseline[metric_key], rerank[metric_key])
                row.append(improvement)
            improvements.append(row)

    df_improvements = pd.DataFrame(improvements, index=configs[:len(improvements)], columns=metrics)
    ...
    ax.set_title('Reranker Performance Improvement (%)', fontsize=13, fontweight='bold', pad=20)
    ax.set_xlabel('Metric', fontsize=11, fontweight='bold')
    ax.set_ylabel('Configuration', fontsize=11, fontweight='bold')

    plt.tight_layout()
    plt.savefig('figure2_improvement_heatmap.png', bbox_inches='tight', dpi=300)
    plt.close()
    print("✓ Figure 2: Improvement heatmap saved")


def plot_radar_chart(all_data):
    ...
    fig.suptitle('Radar Chart: Baseline vs Reranker Performance', fontsize=14, fontweight='bold', y=0.98)
    ...
    for config_key, config_name, ax in configs:
        ...
        if baseline and rerank:
            ...
            ax.fill(angles, baseline_values, alpha=0.15, color=COLORS['baseline'])

            # Plot rerank
            ax.plot(angles, rerank_values, 'o-', linewidth=2, label='Rerank', color=COLORS['rerank'])
            ax.fill(angles, rerank_values, alpha=0.15, color=COLORS['rerank'])

            # Add value labels
            for angle, bl_val, rr_val in zip(angles[:-1], baseline_values[:-1], rerank_values[:-1]):
                ax.text(angle, bl_val + 0.02, f'{bl_val:.3f}', ha='center', va='center', fontsize=7, color=COLORS['baseline'])
                ax.text(angle, rr_val - 0.02, f'{rr_val:.3f}', ha='center', va='center', fontsize=7, color=COLORS['rerank'])
...
def plot_latency_comparison(all_data):
    ...
    bars1 = ax.bar(x - width/2, baseline_latencies, width, label='Baseline', color=COLORS['baseline'], alpha=0.8)
    bars2 = ax.bar(x + width/2, rerank_latencies, width, label='Rerank', color=COLORS['rerank'], alpha=0.8)
    ...
    for i, (bar1, bar2) in enumerate(zip(bars1, bars2)):
        ...
        ax.text(bar1.get_x() + bar1.get_width()/2., height1, f'{height1:.0f}', ha='center', va='bottom', fontsize=9)
        ax.text(bar2.get_x() + bar2.get_width()/2., height2, f'{height2:.0f}', ha='center', va='bottom', fontsize=9)

        # Add improvement arrow
        if improvement < 0:
            ax.annotate(f'↓ {abs(improvement):.1f}%',
                       xy=(bar2.get_x() + bar2.get_width()/2., height2),
                       xytext=(bar2.get_x() + bar2.get_width()/2., height2 + 300),
    ...
    ax.set_title('Latency Comparison: Baseline vs Reranker', fontsize=13, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('figure4_latency_comparison.png', bbox_inches='tight', dpi=300)
    plt.close()
    print("✓ Figure 4: Latency comparison saved")


def plot_comprehensive_summary(df_plot, all_data):
    """Figure 5: Comprehensive summary with all metrics and improvements."""
    fig = plt.figure(figsize=(16, 10))
    gs = fig.add_gridspec(2, 3, hspace=0.3, wspace=0.3)

    # 5a: Overall performance comparison
    ax1 = fig.add_subplot(gs[0, :2])
    metrics_avg = df_plot.groupby(['Config', 'Type'])['Score'].mean().unstack()
    x = np.arange(len(metrics_avg.index))
    width = 0.35

    bars1 = ax1.bar(x - width/2, metrics_avg['Baseline'], width, label='Baseline', color=COLORS['baseline'], alpha=0.8)
    bars2 = ax1.bar(x + width/2, metrics_avg['Rerank'], width, label='Rerank', color=COLORS['rerank'], alpha=0.8)
    ...
    # 5b: Best performing configuration
    ax2 = fig.add_subplot(gs[0, 2])
    best_config = df_plot[df_plot['Type'] == 'Rerank'].groupby('Config')['Score'].mean()
    best_config_sorted = best_config.sort_values(ascending=True)

    colors = ['#95a5a6', '#7f8c8d', '#2c3e50']
    bars = ax2.barh(range(len(best_config_sorted)), best_config_sorted.values, color=colors)
    ...
    ax2.set_yticklabels([c.split('(')[1].split(')')[0] for c in best_config_sorted.index])
    ...
    # 5c: Metric-specific improvements
    ax3 = fig.add_subplot(gs[1, 0])
    metric_improvements = {}
    for metric in ['ROUGE-1', 'ROUGE-2', 'ROUGE-L', 'BERTScore']:
        baseline_avg = df_plot[(df_plot['Metric'] == metric) & (df_plot['Type'] == 'Baseline')]['Score'].mean()
        rerank_avg = df_plot[(df_plot['Metric'] == metric) & (df_plot['Type'] == 'Rerank')]['Score'].mean()
        improvement = ((rerank_avg - baseline_avg) / baseline_avg) * 100
        metric_improvements[metric] = improvement

    colors = [COLORS['rouge1'], COLORS['rouge2'], COLORS['rougel'], COLORS['bertscore']]
    bars = ax3.bar(metric_improvements.keys(), metric_improvements.values(), color=colors, alpha=0.7)

    for bar in bars:
        height = bar.get_height()
        ax3.text(bar.get_x() + bar.get_width()/2., height, f'{height:.2f}%', ha='center', va='bottom', fontsize=8)
    ...
    # 5d: Latency vs Performance tradeoff
    ...
    for config_key, config_name in config_names:
        ...
        if baseline and rerank:
            latency_data.extend([baseline['latency_ms'], rerank['latency_ms']])
            perf_baseline = (baseline['aggregate/avg_rougel'] + baseline['aggregate/avg_bertscore_f1']) / 2
            perf_rerank = (rerank['aggregate/avg_rougel'] + rerank['aggregate/avg_bertscore_f1']) / 2
            performance_data.extend([perf_baseline, perf_rerank])
            config_labels.extend([f'{config_name}\nBaseline', f'{config_name}\nRerank'])

    ax4.scatter(latency_data, performance_data, c=[COLORS['baseline'], COLORS['rerank']] * 3, s=200, alpha=0.7, edgecolors='black', linewidths=1.5)

    for i, label in enumerate(config_labels):
        ax4.annotate(label.split('\n')[0], (latency_data[i], performance_data[i]), xytext=(5, 5), textcoords='offset points', fontsize=7)
    ...
    avg_latency_red = np.mean([((rerank_latencies[i] - baseline_latencies[i]) / baseline_latencies[i]) * 100 for i in range(len(baseline_latencies))])
    summary_text += f"  Average: {avg_latency_red:.2f}%"

    ax5.text(0.1, 0.9, summary_text, transform=ax5.transAxes, fontsize=10, verticalalignment='top', family='monospace', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.3))

    plt.suptitle('Comprehensive Performance Analysis', fontsize=14, fontweight='bold', y=0.995)
    plt.savefig('figure5_comprehensive_summary.png', bbox_inches='tight', dpi=300)
    plt.close()
...
def generate_summary_report(all_data, df_plot):
    ...
    for config_key, config_name in config_names:
        ...
        if baseline and rerank:
            report.append(f"\n{config_name}:")
            report.append(f"  Baseline ROUGE-L: {baseline['aggregate/avg_rougel']:.4f}")
            report.append(f"  Rerank ROUGE-L: {rerank['aggregate/avg_rougel']:.4f}")
            rouge_imp = calculate_improvement(baseline['aggregate/avg_rougel'], rerank['aggregate/avg_rougel'])
            report.append(f"  Improvement: {rouge_imp:.2f}%")
            report.append(f"  Baseline BERTScore: {baseline['aggregate/avg_bertscore_f1']:.4f}")
            report.append(f"  Rerank BERTScore: {rerank['aggregate/avg_bertscore_f1']:.4f}")
            bert_imp = calculate_improvement(baseline['aggregate/avg_bertscore_f1'], rerank['aggregate/avg_bertscore_f1'])
            report.append(f"  Improvement: {bert_imp:.2f}%")
    ...
    for config_key, config_name in config_names:
        ...
        if baseline and rerank:
            latency_change = calculate_improvement(baseline['latency_ms'], rerank['latency_ms'])
            report.append(f"\n{config_name}:")
    ...
    best_config = df_plot[df_plot['Type'] == 'Rerank'].groupby('Config')['Score'].mean().idxmax()
    ...
    report.append(f"• Most improved metric: {best_metric} ({metric_improvements[best_metric]:.2f}%)")
...
def main():
    ...
    plot_grouped_bar_comparison(df_plot)
    plot_improvement_heatmap(all_data)
    plot_radar_chart(all_data)
    plot_latency_comparison(all_data)
    plot_comprehensive_summary(df_plot, all_data)
